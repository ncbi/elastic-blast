...
from elb import taxonomy
from elb import constants
from elb import filehelper
from elb.util import UserReportError
from elb.constants import ELB_QUERY_BATCH_DIR, ELB_TAXIDLIST_FILE
...
def wait(mocker):
    ...
    def mocked_entrez_query(tool, query):
        """Mocked elb.taxonomy.entrez_query: waits 1 second and calls the
        original function"""
        time.sleep(1)
        return orig_entrez_query(tool, query)

    mocker.patch('elb.taxonomy.entrez_query', side_effect=mocked_entrez_query)
    yield
...
def mocked_get_machine_properties(mocker):
    ...
    mocker.patch('elb.elb_config.aws_get_machine_properties', side_effect=fun_mocked_get_machine_properties)
...
def test_get_user_taxids_errors():
    ...
    assert '-taxids and -taxidlist options are mutually exclusive' in err.value.message
...
def test_get_species_taxids(wait):
    ...
    assert sorted(species_taxids) == sorted(expected_taxids)
