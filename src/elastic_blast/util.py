...
def safe_exec_print(cmd: Union[List[str], str], env: Optional[Dict[str, str]] = None) -> subprocess.CompletedProcess:
    """Wrapper around subprocess.run that raises SafeExecError on errors from
    command line with error messages assembled from all available information
    ...
    try:
        ...
        p = subprocess.Popen(cmd, stdout=subprocess.PIPE,
                           stderr=subprocess.PIPE, env=run_env, universal_newlines=True, text=True)
